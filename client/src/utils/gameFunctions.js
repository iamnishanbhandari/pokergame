export function getHand(pDeck, hDeck) {
  ...
  const playerDeck = pDeck.filter((item) => {
    if (item) return item;
  });
  const houseDeck = hDeck.filter((item) => {
    if (item) return item;
  });
  ...
  const combined =
    playerDeck &&
    houseDeck &&
    playerDeck.concat(houseDeck).map((item) => {
      if (!item) return;
  ...
  const values = combined.map((item) => {
    return item.value;
  });
  const descending = values.sort((a, b) => {
    return b - a;
  });
  const suits = combined.map((item) => {
    return item.suit;
  });
  ...
  combined.forEach(({ suit, value }) => {
    ...
    const StraightFlush =
      combined.some((e) => {
        return e.value === value + 1 && e.suit === suit;
      }) &&
      combined.some((e) => {
        return e.value === value + 2 && e.suit === suit;
      }) &&
      combined.some((e) => {
        return e.value === value + 3 && e.suit === suit;
      }) &&
      combined.some((e) => {
        return e.value === value + 4 && e.suit === suit;
      });
    ...
    const SteelWheel =
      combined.some((e) => {
        return e.value === 14 && e.suit === suit;
      }) &&
      combined.some((e) => {
        return e.value === 2 && e.suit === suit;
      }) &&
      combined.some((e) => {
        return e.value === 3 && e.suit === suit;
      }) &&
      combined.some((e) => {
        return e.value === 4 && e.suit === suit;
      }) &&
      combined.some((e) => {
        return e.value === 4 && e.suit === suit;
      });
  ...
  if (
    ...
    const descendingFiltered = descending.filter((e) => {
      return e !== hand.primary;
    });
    ...
    hand.tertiary = descending.filter((e) => {
      return e !== hand.primary && e !== hand.secondary;
    })[0];
  } else if (twoMatches.length > 2) {
    hand.type = "Two Pair";
    ...
    hand.secondary = descending.filter((e) => {
      return e !== hand.primary;
    })[0];
  ...
  if (rank1 > rank2) return p1;
    ...
    if (rank1 !== "Royal Flush") {
      if (hand1.primary > hand2.primary) return p1;
        ...
        if (hand1.secondary) {
          // checks if secondary criteria exists
          ...
          if (hand1.secondary > hand2.secondary) return p1;
            ...
            if (hand1.tertiary) {
              // checks if tertiary criteria exists
